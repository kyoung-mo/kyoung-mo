...
| 기간 | 활동 / 경력 / 수상 내역 | 주요 내용 |
|---|---|---|
| 2025.12 ~ 2026.07 | Intel Edge AI SW 아카데미 | Intel 9기 **기수 대표**, 주요 프로젝트 PM 3회, STM32 프로젝트 4회 |
| 2020.03 ~ 2026.02 | 강원대학교(춘천) 졸업 | 디지털회로실험 조교, 전기전자공학과 전공 동아리장 |
| 2025.02 ~ 2025.08 | 지능형시스템최적화 연구실 (학부연구생) | CAN 이상탐지, Hailo 엣지 AI 최적화 연구 |
| 2026.07 | 🥇 Intel 과정 프로젝트 경진대회 대상 | 가변 실내 PBV |
...
### 1. [CAN 기반 가변 실내 PBV](https://github.com/kyoung-mo/pbv-cabin) ( 🥇 최종 프로젝트 경진대회 대상 )
> 5노드 분산 CAN 기반 차량의 가변 실내(Reconfigurable Cabin) 환경 제어 (PM)
- CAN(500kbps) : RPi5 Supervisor + STM32 Zone ECU 3개, Vector CANdb++ DBC 설계(좌석 명령/상태 분리)
- 안전 인터록 : 입력 → 승인/거부 판정 → CAN 구동 단방향 흐름, 기어 P에서만 모드 전환 허용
- 8축 좌석 제어(앞 리클라인·회전 / 뒤 리클라인·슬라이드) + 4개 실내 모드 프리셋(주행·회의·Full-space·휴식)
- QT6 QML 기반 실시간 디지털 트윈 HMI(좌석 명령/피드백 3D 시각화, X-ray 차체·Ambient 모드)
